...
import {
  CategoryChartIcon as CategoryIcon,
  TrendChartIcon,
  ICON_STROKE,
            ...
            <span>
              ...
              {topSlice ? (
                <span className="mt-1 block text-sm text-muted-foreground">
                  ...
                  {Math.round((topSlice.amount / total) * 100)}% {t("ofSpending")}
                </span>
              ) : null}
            </span>
        ...
        <>
          <div className="mb-4 flex items-baseline justify-between gap-3 border-b border-border/70 pb-3">
            ...
            <span
              className={
                latestNet >= 0
                  ? "wazen-number text-sm text-chart-2"
                  : "wazen-number text-sm text-destructive"
              }
            >
              {latestNet >= 0 ? "+" : "−"}
              {formatMoney(Math.abs(latestNet), currency)}
            </span>
          </div>
          <div
            ref={ref}
            ...
            {revealed ? (
              <ResponsiveContainer width="100%" height="100%">
                <ComposedChart
                  data={series}
                  barGap={3}
                  margin={{ top: 8, right: 2, bottom: 0, left: -28 }}
                >
                  <CartesianGrid
                    vertical={false}
                    stroke="var(--border)"
      ...
      {!hasData ? (
        ...
        <>
          <div className="mb-4 flex items-baseline justify-between gap-3 border-b border-border/70 pb-3">
            ...
            <span className="wazen-number text-sm text-chart-2">
