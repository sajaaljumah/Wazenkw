...
function KidProgress({ percent, className }: { percent: number; className?: string }) {
  ...
  return (
    <div
      ref={ref}
      className={cn(
        "wazen-progress-track h-5 w-full overflow-hidden rounded-full bg-kid-soft/70",
        className,
      )}
...
const ACTIVITY_KEY: Record<
  Transaction["kind"],
  "kidGotMoney" | "kidSpentWord" | "kidSavedMoneyWord" | "kidMoneyBack"
> = {
  income: "kidGotMoney",
...
export function ChildDashboard({
  ...
  const monthTransactions = useMemo(
    () => inMonth(transactions, monthKey(new Date())),
    [transactions],
  );
      ...
      <section className="kid-panel relative order-0 overflow-hidden bg-kid-tint p-7 sm:p-9">
        ...
        <div className="relative grid grid-cols-1 items-center gap-5 text-center sm:grid-cols-[auto_minmax(0,1fr)_auto] sm:gap-7 sm:text-start">
          ...
          <div className="min-w-0">
            <p className="text-sm text-kid-deep">
              {t("kidHi")} {firstName}!
            </p>
          ...
          >
            ...
            <span className="min-w-0 flex-1">
              ...
              <span className="mt-3 block text-sm text-muted-foreground">
                {formatMoney(saved, currency)} {t("kidGoalSavedOf")} {formatMoney(target, currency)}{" "}
                — {t("kidGoalThere")} {percent}% {t("kidGoalThereEnd")}
              </span>
      ...
      <section className="wazen-reveal order-2 space-y-4" data-revealed={r2.revealed} ref={r2.ref}>
        ...
        <div className="grid gap-4 min-[520px]:grid-cols-3">
          ...
          <ChoiceTile
            ...
            caption={
              give > 0
                ? `${formatMoney(give, currency)} ${t("kidSharedCaption")}`
                : t("kidLearnSharing")
            }
              ...
              {recent.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span
                      className={cn(
                        "col-start-2 text-sm tabular-nums min-[420px]:col-start-3",
                        isOut ? "text-kid-deep" : "text-foreground",
                      )}
                    >
                      {isOut ? "−" : "+"}
      ...
      <div className="space-y-4 text-sm">
        <div className="flex items-center gap-4 rounded-3xl bg-secondary/50 p-4">
          ...
          <p>
            ...
            <span className="tabular-nums">{formatMoney(given, currency)}</span>{" "}
            {t("kidGiveSummaryEnd")}
          </p>
        </div>
        <ul className="space-y-2 text-muted-foreground">
          <li className="flex gap-2">
            <GiveIcon className="mt-0.5 size-4 shrink-0" strokeWidth={ICON_STROKE} />{" "}
            {t("kidGiveTip1")}
          </li>
          <li className="flex gap-2">
            <ScheduledIcon className="mt-0.5 size-4 shrink-0" strokeWidth={ICON_STROKE} />{" "}
            {t("kidGiveTip2")}
          </li>
          <li className="flex gap-2">
            <PremiumIcon className="mt-0.5 size-4 shrink-0" strokeWidth={ICON_STROKE} />{" "}
            {t("kidGiveTip3")}
          </li>
        </ul>
        <button
