    ...
    <Dialog open={open} onOpenChange={(next) => (!next ? complete() : undefined)}>
      ...
      >
        <div className="bg-accent/55 px-6 pb-7 pt-8 sm:px-8">
          ...
          <p className="wazen-label mt-6">
            {t("tourStep")} {step + 1} {t("ofWord")} {steps.length}
          </p>
          <DialogTitle className="mt-2 text-2xl sm:text-3xl">{current.title}</DialogTitle>
          <DialogDescription id="walkthrough-description" className="mt-3 text-sm leading-relaxed">
            {current.body}
          </DialogDescription>
        </div>
        <div className="px-6 pb-6 sm:px-8">
          <div className="mb-6 flex gap-2" aria-hidden>
            {steps.map((item, index) => (
              <span
                key={item.title}
                className={`h-1 flex-1 rounded-full ${index <= step ? "bg-primary" : "bg-secondary"}`}
              />
            ))}
          </div>
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            <div className="flex gap-2">
              ...
              <Button
                onClick={() =>
                  step === steps.length - 1 ? complete() : setStep((value) => value + 1)
                }
              >
